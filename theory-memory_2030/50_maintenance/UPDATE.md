...
人間向けの読み物としての体裁より、AIの理解速度・トークン効率を優先する。目標は単なる文字数削減ではなく、少ない読み込みで正確に議論を再開できること。

- 追記前に既存の記載先を確認し、結論・成立条件・変更点・未解決点を該当箇所へ短く統合する。対話の順序や同じ説明・留保を繰り返し保存しない。
- `00_START`は最小の再開情報、モジュールは論点別の統合、対話記録・記事カード・原文は必要時の詳細とする。層をまたぐ再開用要約は短く保ち、詳細の重複はリンクや主張IDへ置き換える。
- 著者/ユーザー/整理者の帰属、仮説/確認事項の区別、重要な条件・反例・数値の版差、旧説からの変更関係は圧縮後も保持する。原文保存・版管理の規則を優先する。
- 未定義の略語や過密な記号化で理解を遅くしない。更新後は入口から必要情報へ短く到達できるか、不要な全文読み込みや参照往復を増やしていないかを確認する。
